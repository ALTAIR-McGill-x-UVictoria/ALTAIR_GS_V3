...
function CompassRose({ azimuth, elevation }) {
  ...
  const cx  = 80
  const cy  = 80
  // Arrow tip coordinates (pointing in azimuth direction)
  ...
  return (
    <svg width={160} height={160} style={{ display: 'block', margin: '0 auto' }}>
      ...
      {/* Elevation arc label */}
      <text x={cx} y={cy + r + 28} textAnchor="middle" fill={C.yellow} fontSize={10} fontFamily="monospace">
        El {el.toFixed(1)}°
      </text>
    </svg>
  )
    ...
    <div style={styles.root}>
      ...
      <div style={styles.column}>
        ...
        <Section title="Mount">
          ...
          {/* Mount type selector + port + connect/disconnect */}
          <div style={{ ...styles.inputRow, marginTop: 8 }}>
            ...
            {mountConnected
              ? <button style={styles.btnDanger} disabled={busy}
                  onClick={() => run(actions.disconnectMount)}>
                  Disconnect
                </button>
              : <button style={styles.btn} disabled={busy}
                  onClick={() => run(() => actions.connectMount(mountType, mountPort))}>
                  Connect
                </button>
            }
          </div>

          {/* Manual goto — fields depend on mount type */}
          {activeMountType === 'am5'
            ? (
              <div style={{ ...styles.inputRow, marginTop: 8 }}>
                <input
        ...
        <Section title="Camera">
          ...
          <div style={{ ...styles.inputRow, marginTop: 8 }}>
            {cameraConnected
              ? <button style={styles.btnDanger} disabled={busy}
                  onClick={() => run(actions.disconnectCamera)}>
                  Disconnect
                </button>
              : <button style={styles.btn} disabled={busy}
                  onClick={() => run(actions.connectCamera)}>
                  Connect
                </button>
            }
          </div>
              ...
              <div style={{ ...styles.inputRow, marginTop: 8 }}>
                ...
                <button
                  ...
                  onClick={() => run(async () => {
                    ...
                    if (res.ok) setLastCapture(res.path)
                  })}
                >
                  Capture
                </button>
      ...
      {/* ---- Right column: tracking geometry ---- */}
      <div style={styles.column}>
